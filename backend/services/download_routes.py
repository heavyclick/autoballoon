...
from services.download_service import download_service, DownloadResult
...
class PageData(BaseModel):
    ...
    page_number: int
    image: str  # base64 encoded PNG (can also use image_base64)
    width: int
    height: int
...
async def download_ballooned_pdf(request: DownloadRequest):
    ...
    Returns a PDF file with balloon markers rendered on each page.
    """
    # Build metadata
    metadata = None
...
async def download_zip_bundle(request: DownloadRequest):
    ...
    Contents:
    - images/ folder with ballooned PNG for each page
    - AS9102 Form 3 Excel file with all dimensions
...
async def download_single_image(request: SingleImageDownloadRequest):
    ...
    Returns a PNG or JPEG image with balloon markers rendered.
    """
...
async def download_excel_only(request: DownloadRequest):
    ...
    Convenience endpoint for getting just the Excel file.
    """
    from services.export_service import export_service
    from models import ExportFormat, ExportTemplate
